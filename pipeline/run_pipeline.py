...
# RePaint 流程
# ---------------------------------------------------------
def run_repaint_pipeline(args, imgs, real_dir, mask_dir, corrupted_dir, fake_dir):
    """
    ...
    for p in imgs:
        ...
        if not os.path.exists(m) or args.overwrite:
...
# Copy-Move 流程
# ---------------------------------------------------------
def run_copymove_pipeline(args, imgs, real_dir, mask_dir, fake_dir):
    """
    Copy-Move 伪造流程：mask -> copy-move
    """
    print("\n" + "=" * 70)
    ...
    for img_path in imgs:
        ...
        try:
            ...
            print(f"  [6/6] 完成：保存 {saved_cnt}/{len(fake_images)} 张伪造图像")
            success_cnt += 1
    ...
    print(f"\nCopy-Move 流程完成：成功 {success_cnt}，失败 {fail_cnt}")
    print(f"输出目录：{fake_dir}")
    print("=" * 70)
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=2, help="[RePaint] 批次大小")
    
    # Copy-Move 专用参数
    parser.add_argument("--samples-per-image", type=int, default=5, help="[Copy-Move] 每张图片生成的伪造样本数")
    ...
    # 执行对应的流程
    if args.type == "repaint":
        run_repaint_pipeline(args, imgs, real_dir, mask_dir, corrupted_dir, fake_dir_repaint)
    elif args.type == "copymove":
        run_copymove_pipeline(args, imgs, real_dir, mask_dir, fake_dir_copymove)
